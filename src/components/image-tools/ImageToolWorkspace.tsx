...
const COMPRESSOR_CONFIG: FileToolConfig = {
  ...
  description: "Reduce a JPEG or PNG locally, then compare the result before downloading.",
  uploadLabel: "Drop your image here or browse",
  uploadHelperText: "Choose one JPEG or PNG image to compress",
...
type CompressionOptions = { quality: number };

const compressImage: FileProcessor<CompressionOptions> = async (context) => {
  const item = requireInput(context);
  const format = imageFormatFromFile(item.file);
  if (!format) throw new FileToolProcessingError("Choose a valid JPEG or PNG image.", { retryable: false });
  reportStep(context, item.id, 8, "Decoding the image locally…");
  const decoded = await decodeImage(item.file, context.signal);
  try {
    reportStep(context, item.id, 45, "Rendering the original dimensions…");
    const canvas = drawResizedImage(decoded, { width: decoded.width, height: decoded.height }, format);
    ...
    return {
      outputs: [{
        blob,
        fileName: buildImageFileName(item.file.name, "compressed", format),
        mimeType: IMAGE_MIME_BY_FORMAT[format],
        originalSize: item.file.size,
        metrics: [{ label: "Dimensions", value: formatDimensions({ width: decoded.width, height: decoded.height }) }],
      }],
      metrics: [
...
const convertImage: FileProcessor<ConvertOptions> = async (context) => {
  ...
  try {
    ...
    reportStep(context, item.id, 48, `Preparing ${context.options.format === "png" ? "PNG" : "JPG"} pixels…`);
    ...
    return {
      ...
      metrics: [
        { label: "Format", value: context.options.format === "png" ? "PNG" : "JPG" },
        { label: "Dimensions", value: formatDimensions(dimensions) },
        { label: "Output size", value: formatBytes(blob.size) },
      ],
      summary: `The ${context.options.format === "png" ? "PNG" : "JPG"} was created locally without changing the pixel dimensions.`,
    };
  } finally {
    decoded.dispose();
  }
...
function CompressorTool() {
  ...
  const workflow = useFileWorkflow(COMPRESSOR_CONFIG, compressImage, { quality });
  ...
  const format = file ? imageFormatFromFile(file) : null;
  return (
    <ToolPageFrame active="image-compressor">
      <FileToolView
        ...
        optionsPanel={file ? <>
          ...
          <OptionsSection icon={<ImageDown aria-hidden="true" size={16} />} title="Compression strength">
            {format === "jpeg" ? <QualityControl onChange={setQuality} value={quality} /> : <p className={styles.infoLine}><Info aria-hidden="true" size={15} /> PNG uses lossless browser re-encoding, so output can occasionally be larger.</p>}
            {format === "jpeg" ? <div className={styles.presetButtons}>
              {[{ label: "Light", value: .86 }, { label: "Balanced", value: .72 }, { label: "Strong", value: .55 }].map((preset) => <button aria-pressed={quality === preset.value} key={preset.label} onClick={() => setQuality(preset.value)} type="button">{preset.label}</button>)}
            </div> : null}
          </OptionsSection>
        </> : undefined}
        resultInfoSlot="Compression results depend on image content. The original file is never modified."
        resultPreviewSlot={<ResultImage alt="Compressed result preview" result={workflow.state.result} />}
